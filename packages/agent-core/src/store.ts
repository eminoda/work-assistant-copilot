...
import { workflowSchema, type ExecutionResult, type Workflow } from '@workcopilot/workflow-engine'
...
export class WorkCopilotStore {
  ...
  async listWorkflows(): Promise<Array<Workflow & { id: string }>> {
    ...
    return rows.map((row) => workflowSchema.parse({
      id: row.id, name: row.name, intent: row.intent, description: row.description,
      version: 1, steps: row.steps,
  ...
  async getWorkflow(id: string): Promise<(Workflow & { id: string }) | undefined> {
    ...
    return workflowSchema.parse({ id: row.id, name: row.name, intent: row.intent, description: row.description, version: 1, steps: row.steps }) as Workflow & { id: string }
  }
  async saveWorkflow(input: Workflow): Promise<Workflow & { id: string }> {
    ...
    const row = await this.db.workflow.create({
      data: { name: workflow.name, intent: workflow.intent, description: workflow.description, steps: workflow.steps as Prisma.InputJsonValue },
    })
    return { ...workflow, id: row.id }
  }
